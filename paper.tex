...
Another seemingly paradoxical observation is also possible:
...
opposed to merely constructive one. In particular, our framework is
...
already anti-classical, as the undecidability of the halting program can be
observed internally.
...
at which an infinite number of atoms have already been generated; in this way,
...
\begin{remark}
  One way to recover something essentially equivalent to the supervaluation
  semantics in our style would be to replace the current semantic universe
  $\Define{\SemU}{\Classes^\SCtx}$ with an extended universe
  $\Define{\SemU_\infty}{\Classes^{\SCtx_\infty}}$, $\SCtx_\infty$ the set of
  possibly-infinite symbol contexts and injective, sort-preserving renamings.
  Externally, by Theorem~\ref{thm:no-nat-injection} we could still never
  exhibit an embedding of the naturals, but internally, we would be unable to
  rule it out.
\end{remark}
