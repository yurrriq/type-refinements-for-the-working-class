...
\begin{definition}[General Judgment]
  For a metavariable context $\IsMetaCtx{\Omega}$, we can define the presheaf
  of its substitutions $\Of{\square\Omega}{\Sets^\SCtx}$ as follows:
  \[
    \Define{{\square\Omega}\,(\Upsilon)}{
      \prod_{(\MV{m}:v)\in\Omega}
        \MkSet{E\mid\IsAbtUnmoded{\cdot}{\Upsilon}{\cdot}{E}{v}}
    }
  \]

  For any diagram $\Of{\mathcal{J}}{\mathbb{J}^{\square\Omega(\Upsilon)}}$, we can define
  ...
  $\Of{\Yoneda{-}}{\mathcal{W}\to\mathbb{J}}$ is the Yoneda Embedding:
   ...
   (i.e. for any substitution $\Member{\omega}{\square\Omega(\Upsilon)}$,
