...
\begin{definition}[Internal Judgments]
  ...
  using the power object $\Pow{-}$: for any judgment
  $\mathcal{J}\in\mathbb{J}$, the derivations of $\Pow{\mathcal{J}}$ shall
  consist in subsets of the derivations of $\mathcal{J}$. The evidence of
  $\Pow{\mathcal{J}}$ shall be called an \emph{internal judgment} $J$. In most
  cases, we will say something like ``To know $\mathcal{J}$ is to have a
  judgment $\mathcal{J'}$'' rather than appealing directly to the power object
  or the subobject classifier.
\end{definition}
...
presheaves of closed expressions and closed values over symbol contexts and sorts as
follows:
\begin{align*}
  \ADefine{\Exprs_\tau(w)}{
    \MkSet{M \mid \IsAbtUnmoded{\cdot}{\pi(w)}{\cdot}{M}{\tau}}
  }\\
  \ADefine{\Values_\tau(w)}{
    \MkSet{
      M\equiv\App{\vartheta}{\vec{E}}
      \mid M\in\Exprs_\tau(w)\land \exists a. \CanOperators\Pair{\pi(w)}{a}\ni\vartheta
    }
  }
\end{align*}
...
$\Match{L}{\Pair{\Sigma}{\CanOperators}}$ along with a
$\equiv_\alpha$-functional evaluation relation. Formally, the evaluation
relation is given in our logical framework by a sort-indexed family of
intensional sets of evaluation rules
$\Of{\mathfrak{R}^\tau_\Downarrow}{\mathbb{J}}$, such that the judgment
$\HypJ{\Pow{\mathfrak{R}^\tau_\Downarrow}}{\Exprs_\tau, \Values_\tau,
\NaturalsPsh}$ shall be evident by means of some internal family of judgments
$[\Downarrow_\tau]$, where $\NaturalsPsh$ is the constant presheaf
$\MapsTo{w}{\Naturals}$. In other words, we shall forevermore know what it
means to know that some expression evaluates to some value in $n$ steps.  By
this token, we write $\EvalN{\Upsilon}{\tau}{n}{M}{N}$ to mean the judgment
whose derivations at $w$ are the set $[\Downarrow_\tau](w,M,N,n)$.
