...
valence, we will say $\IsValence{v}$. An $\Sorts$-arity is of the form
$\MkArity{\vec{v}}{\tau}$, where $\vec{v}$ are $\Sorts$-valences and
$\IsSort{\tau}$; arities classify \emph{operators} (also called ``function
symbols'' in older treatments of algebra). When $a$ is an arity, we will say
$\IsArity{a}$.

An \emph{operator signature} is an arity-indexed family of copresheaves on
...
of fibering over symbol contexts is to introduce an open-ended indexing
...
% \todo[inline]{give a brief description of the framework, and present its rules.}
