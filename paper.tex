...
Then, we can define a refinement $\GloballyRefines{\mathbb{A}_0}{\SortAtom}$ as
follows:
\[
  \infer{
    \RMemEq{\Upsilon}{
...
However, it is not the case that $\mathbb{A}_0$ is a type, because it doesn't
respect computational equivalence. This is easily resolved by defining the type
of atoms as the computational extension of $\mathbb{A}_0$,
\[
  \Define{
    \mathbb{A}
  }{
    \VToCType(\mathbb{A}_0)
  }
\]

\begin{thm}
  For any $\Upsilon$, we have $\IsType{\Upsilon}{\mathbb{A}}{\SortAtom}$.
\end{thm}
\begin{proof}
...
For instance, assuming a type $\Naturals$ of natural numbers and $\mathbb{A}$
of atoms, with $\Naturals\inj\mathbb{A}$ the type of embeddings of the naturals
into the atoms, we do have
$\NuprlInterpSeq{\Strings}{\cdot}{\Naturals\inj\mathbb{A}}$, because there are
...
$\NuprlInterpSeq{*}{\cdot}{\Naturals\inj\mathbb{A}}$.  Furthermore, we also do
not have $\NuprlInterpSeq{*}{\iota:\Naturals\inj\mathbb{A}}{\bot}$, because this
...
\begin{thm}
  ...
  $\not\models\NuprlSeq{\cdot}{\Naturals\inj\mathbb{A}}$.
\end{thm}
\begin{proof}
  Suppose $\models\NuprlSeq{\cdot}{\Naturals\inj\mathbb{A}}$. Then, there
  exists an $\Member{\Upsilon}{\SCtx}$ and $\Member{M}{\Exprs_\tau(\Upsilon)}$
  such that $\RMemEq{\Upsilon}{M}{M}{\Naturals\inj\mathbb{A}}$; but this
...
\begin{thm}
  ...
  $\models\NuprlSeq{\iota:\Naturals\inj\mathbb{A}}{\bot}$.
\end{thm}
\begin{proof}
  It suffices to show that for all $\Upsilon$, there is no $\iota$ such that
  $\RMemEq{\Upsilon}{\iota}{\iota}{\Naturals\inj\mathbb{A}}$. However, by the
  definition of $\SCtx$, all $\Member{\Upsilon}{\SCtx}$ are finite, so there
  can be no injection of the naturals into $\Dom{\Upsilon}$.
\end{proof}

The crux of the problem is that all worlds (symbol contexts) are finite in our
semantics, so it is possible to observe that it will never be possible to
inject the naturals into the atoms. One way to make our system more open-ended
is to replace the current semantic universe $\Define{\SemU}{\Classes^\SCtx}$
with an extended universe $\Define{\SemU_\infty}{\Classes^{\SCtx_\infty}}$ with
$\SCtx_\infty$ the set of possibly-infinite symbol contexts and injective,
sort-preserving renamings. Externally, by Theorem~\ref{thm:no-nat-injection} we
could still never exhibit an embedding of the naturals, but internally, we
would be unable to rule it out.
