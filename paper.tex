...
\begin{definition}[Internal Judgments]
  ...
  $\Upsilon'$ for which we have $\Of{\rho}{\Upsilon\inj\Upsilon'}$.  A
  \emph{global internal judgment} is a section
  $\Of{J}{\int_\Upsilon\Hom{\mathbb{J}}{\Yoneda{\Upsilon}}{\Omega}}$, which can
  be externalized as a judgment $\Of{\Embed{J}}{\mathbb{J}}$ whose derivations
  at $\Upsilon$ are the arrows
  $\Of{\phi}{\Hom{\mathbb{J}}{\Yoneda{\Upsilon}}{\mathbbold{1}}}$ such that the
  following diagram commutes:
  \[
    \begin{tikzcd}
      \IMode{\Yoneda{\Upsilon}}
        \arrow[r, dashed, "\OMode{\phi}"]
        \arrow[rd, "\IMode{J_\Upsilon}"']
      & \IMode{\mathbbold{1}} \arrow[d, "\IMode{\mathbf{true}}"] \\
      & \IMode{\Omega}
    \end{tikzcd}
  \]

  In most cases, we will say something like ``To know $\mathcal{J}$ is to have
  a judgment $\mathcal{J'}$'' rather than appealing directly to the subobject
  classifier.
\end{definition}
...
expressing that $M$ evaluates to $N$ in $n$ steps.

%% \begin{remark}
%%
%%   Formally, the evaluation relation is given in our logical framework by a
%%   presheaf of evaluation rules $\Of{\mathfrak{R}^\tau_\Downarrow}{\mathbb{J}}$,
%%   such that the judgment
%%   $\HypJ{\Pow{\mathfrak{R}^\tau_\Downarrow}}{\Exprs_\tau, \Values_\tau,
%%   \NaturalsPsh}$ is globally evident where $\NaturalsPsh$ is the constant
%%   presheaf $\MapsTo{\Upsilon}{\Naturals}$. In other words, we have a family of
%%   internal judgments $\Of{[\Downarrow_\tau](\Upsilon)}{\int_{\Upsilon'}
%%   \Upsilon\inj\Upsilon'\Rightarrow\Pow{\mathfrak{R}^\tau_\Downarrow}}$
%%
%%
%% shall be evident by means of some internal
%% family of judgments $[\Downarrow_\tau]$, where $\NaturalsPsh$ is the constant
%% presheaf $\MapsTo{\Upsilon}{\Naturals}$. In other words, we shall forevermore know
%% what it means to know that some expression evaluates to some value in $n$
%% steps.
%% \end{remark}
%%
%% We will write $\Eval{\Upsilon}{\tau}{M}{N}$ to mean that there exists an $n$
%% such that $\EvalN{\Upsilon}{\tau}{n}{M}{N}$; in other words, the judgment is
%% interpreted as the following presheaf:
%% \[
%%   \Define{
%%     \left(\Upsilon\parallel M\Downarrow_\tau N\right)(w)
%%   }{
%%     \int^n[\Downarrow_\tau](w,M,N,n)
%%   }
%% \]
%%
%%
%% By this token, the derivations of $\EvalN{\Upsilon}{\tau}{n}{M}{N}$ at
%% $w$ are the set $[\Downarrow_\tau](w,M,N,n)$; we write
%% $\Eval{\Upsilon}{\tau}{M}{N}$ for the judgment
%% $\int^n[\Downarrow_\tau](-,M,N,n)$
%
