...
For any abt signature $\Match{\Sigma}{\Pair{\Sorts}{\Operators}}$ and a worlds
category $\mathcal{W}$, we may deploy a general logical framework in which the
meaning of a judgment is an object of the presheaf category
$\Define{\mathbb{J}}{\Sets^\mathcal{W}}$ (that is, a judgment is identified
with the intensional set of its derivations).

\begin{definition}[General Judgment]
  For a metavariable context $\IsMetaCtx{\Omega}$, we can define the set
  of its substitutions as the following product:
  \[
    \Define{[\Omega]}{
      \prod_{\MV{m}:v} \MkSet{E\mid\IsAbtUnmoded{\cdot}{\cdot}{\cdot}{E}{v}}
    }
  \]
  Let $\Discrete{[\Omega]}$ be the discrete category on $[\Omega]$. Then, for
  any diagram $\Of{\mathcal{J}}{\mathbb{J}^{\Discrete{[\Omega]}}}$, we can
  define the general judgment $\GenJ{\Omega}{\mathcal{J}}$ as follows:
  \[
    \textstyle
    \Define{
      ...
      \Hom{\mathbb{J}}{
        ...
        \varinjlim_{\Discrete{[\Omega]}}\mathcal{J}
      }
    }\qquad
    (\Member{w}{\mathcal{W}})
  \]
  ...
  (i.e. for any substitution of $\Member{\omega}{[\Omega]}$,
  ...
  quantify over terms of higher type.  However, for clarity we will often write
  $\GenJ{\MV{m}:\sigma}{\mathcal{J}}$ rather than
  $\GenJ{\MV{m}:\MkValence{\cdot}{\cdot}{\sigma}}{\mathcal{J}}$; likewise, in
  the body of $\mathcal{J}$ we will simply write $\MV{m}$ rather than the more
  proper $\MApp{\MV{m}}{\cdot}{\cdot}$.
...
\begin{definition}[Hypothetical Judgment]
  ...
  Intuitively, the hypothetical judgment
  $\Define{\HypJ{\mathcal{J}_2}{\mathcal{J}_1}}{\mathcal{J}_1\to\mathcal{J}_2}$
  is evident when there shall forevermore be an effective transformation of the
  evidence into the evidence of the consequent. It is important to keep in mind
  that this is a \emph{semantic} consequence and therefore expresses
  admissibilities, in contrast to the the \emph{logical} consequence which
  expresses derivabilities \cite{harper:2016}. We will not have any need for
  the latter.
\end{definition}
...
\begin{gather*}
  \infer{
    ...
    \GenJ{y,z:\sigma}{
      \HypJ{
        \HEqRefines{\Phi}{\Psi}{\Subst{y}{x}{\phi}}{\Subst{z}{x}{\psi}}{\tau}
      }{
        \HRMemEq{\Phi}{\Psi}{y}{z}{\chi}
      }
    }
  }
\end{gather*}
