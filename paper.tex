...
For any abt signature $\Match{\Sigma}{\Pair{\Sorts}{\Operators}}$ and small worlds
category $\mathcal{W}$, we may deploy a generic logical framework in which the
...
\begin{definition}[General Judgment]
  ...
  (i.e. for any substitution $\Member{\omega}{[\Omega]}$,
...
\begin{definition}[Hypothetical Judgment]
  ...
  contrast to the the \emph{logical} consequence which expresses derivabilities
  \cite{harper:2016}.  We will not have any need for the latter.
\end{definition}
...
character implicit. A judgment will said to be \emph{correct} or \emph{valid}
in case it is globally inhabited.
